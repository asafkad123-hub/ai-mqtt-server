...
# --- SUPABASE CONFIG ---
SUPABASE_URL = "https://unpgjlwtdwzqdhjnfvxm.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVucGdqbHd0ZHd6cWRoam5mdnhtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAwMTg3MTMsImV4cCI6MjA4NTU5NDcxM30.HUTGKJz4FdAHt2VjH_4uUV3Z1WGG_Llj8fQoX9mWfwM"
SUPABASE_TABLE = "bark_analysis_results"
...
def _supabase_insert(row: dict):
    """Insert a row and print id/errors in terminal."""
    try:
        ...
        print(f"📤 Supabase upload success. id={inserted_id}")
...
def analyze():
    ...
    if dog_peak < BARK_THRESHOLD:
        ...
        # Insert Rejection Row
        _supabase_insert({
            "created_at": datetime.now(timezone.utc).isoformat(),
            "device_id": DEVICE_ID,
            "is_bark": False,
            "reject_reason": "yamnet_not_dog",
            "dog_confidence": float(dog_peak)
        })
        return
    ...
    # --- Supabase Upload for the CROPPED result ---
    final_out = out_crop if out_crop["probs_mean"] is not None else out_full
    _supabase_insert({
        "created_at": datetime.now(timezone.utc).isoformat(),
        "device_id": DEVICE_ID,
        "topic": TOPIC,
        ...
        "dog_confidence": float(dog_peak),
        "mamba_pred": final_out["final_mean"],
        "probs": [float(x) for x in final_out["probs_mean"].tolist()] if final_out["probs_mean"] is not None else [],
        "russell_emotion": final_out["russell_mean_label"],
        "valence": final_out["russell_mean_valence"],
        "arousal": final_out["russell_mean_arousal"],
        "consensus_emotion": final_out["final_majority"],
        "is_bark": True,
        "bark_threshold": float(BARK_THRESHOLD)
    })
